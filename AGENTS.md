...
- Control: `Subject Guard` bật/tắt (mặc định **bật**), `Strength` 0–1 (mặc định 0.5, `minThicknessFor(s) = round(3 + (1−s)·13)`, tức 16 px ở 0 và 3 px ở 1), `Leak guard` 0–3 (mặc định 1). `luminanceWeight` lấy từ `Subject Protection` theo cùng công thức keyer dùng (`luminanceWeightFor`), nên "gần key" có cùng nghĩa với cả hai pass.
...
- Lưu trong clip state: `subjectGuardEnabled`, `subjectGuardStrength`, `subjectGuardLeak`. `schemaVersion` vẫn là 5; clip cũ thiếu field ⇒ bật với giá trị mặc định.
